...
class _LevelsDashboardPageState extends State<LevelsDashboardPage> {
                ...
                ? ListView(
                    children: const [
                      Padding(
                        ...
                        child: Center(child: Text('No levels available yet.', style: TextStyle(color: Colors.black54))),
                      )
                    ],
                  )
                        ...
                        children: [
                          ...
                          Column(
                            children: levels.asMap().entries.map((entry) {
                              ...
                              return _buildLevelNode(context, level, index, learning);
                            }).toList(),
                          ),
                        ],
                      ),
  ...
  Widget _buildLevelNode(BuildContext context, LearningLevel level, int index, LearningProvider learning) {
          ...
          child: GestureDetector(
            onTap: isLocked
                ? () => ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Complete previous levels to unlock!')),
                    )
                : () {
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: isActive
                        ? [BoxShadow(color: const Color(0xFF00D1C1).withOpacity(0.4), blurRadius: 15, spreadRadius: 5)]
                        : [],
                  ),
                  child: Icon(
                    isLocked
...
class _RoadPathPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final paint = Paint()
      ..color = const Color(0xFFE0F2F1) // Very light teal background path
      ..strokeWidth = 28 // Thick road
    ...
    for (int i = 1; i < count; i++) {
      ...
      path.cubicTo(
        prevX, prevY + itemHeight / 2.5, // Control point 1 (pulls down from prev)
        currentX, currentY - itemHeight / 2.5, // Control point 2 (pulls up from current)
        currentX, currentY,
      );
    }
...
class _LevelLessonsPageState extends State<_LevelLessonsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final allLessonsDone = lessons.isNotEmpty && lessons.every((lesson) => lesson.isCompleted);
        ...
        children: [
          ...lessons.map(
            (lesson) => Container(
              ...
              decoration: BoxDecoration(
                color: Colors.white,
                border: Border.all(color: lesson.isCompleted ? const Color(0xFF00D1C1) : Colors.black12),
              ...
              child: ListTile(
                ...
                subtitle: Text(
                  ...
                  style: TextStyle(color: lesson.isCompleted ? const Color(0xFF00D1C1) : Colors.black54),
                ),
                trailing: Icon(
                  lesson.isCompleted ? Icons.check_circle_rounded : Icons.play_circle_fill_rounded,
                  color: lesson.isCompleted ? const Color(0xFF00D1C1) : Colors.black26,
                  size: 32,
                ),
                onTap: () {
                  Navigator.of(context).push(
